...
proving closure of atypical and anomalous intersection loci across admissible ambient families through a multi-lane unlikely-intersection super-architecture.

## Main Manuscript

- [paper/UNLIKELY_INTERSECTIONS_PREPRINT.md](paper/UNLIKELY_INTERSECTIONS_PREPRINT.md)
...
5. local-global stitching of the declared atypical-locus endpoint package.
...
- `paper/`
  - `UNLIKELY_INTERSECTIONS_PREPRINT.md`
...
1. Super-lane theorem chain first: read `paper/UNLIKELY_INTERSECTIONS_PREPRINT.md`.
...
Current UIN runner policy:

- `repro/run_repro.sh` extracts, promotes, runs `scripts/uin_closure_guard.py`, updates `repro/repro_manifest.json`, and enforces `fully_extracted` release-gate mode.
...
3. artifact key consumed by `scripts/uin_closure_guard.py`.
...
- Manuscript target: [paper/UNLIKELY_INTERSECTIONS_PREPRINT.md](paper/UNLIKELY_INTERSECTIONS_PREPRINT.md)
