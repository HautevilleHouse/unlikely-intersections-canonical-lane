...
def compute_report(data: dict[str, Any], sigma_star: float | None, strict_coh_zero: bool) -> dict[str, Any]:
    ...
    native_names = ['UIN_G1', 'UIN_G2', 'UIN_G3', 'UIN_G4', 'UIN_G5']
    gates = {name: ('PASS' if passed else 'FAIL') for name, passed in zip(native_names, gate_pass)}
    gates['UIN_G6'] = 'PASS' if coh_pass else 'FAIL'
    gates['UIN_GM'] = 'PASS' if gm_pass else 'FAIL'
    blockers = {}
    ...
    blockers['UIN_G6'] = [] if coh_pass else ['eps_coh missing/invalid or strict-zero target failed']
    blockers['UIN_GM'] = [] if gm_pass else ['strict margin <= 0 or upstream gates failed']
    normalized_gates = {'G1': gates['UIN_G1'], 'G2': gates['UIN_G2'], 'G3': gates['UIN_G3'], 'G4': gates['UIN_G4'], 'G5': gates['UIN_G5'], 'G6': 'NA', 'GCoh': gates['UIN_G6'], 'GM': gates['UIN_GM']}
    normalized_blockers = {'G1': blockers['UIN_G1'], 'G2': blockers['UIN_G2'], 'G3': blockers['UIN_G3'], 'G4': blockers['UIN_G4'], 'G5': blockers['UIN_G5'], 'G6': [], 'GCoh': blockers['UIN_G6'], 'GM': blockers['UIN_GM']}
