...
In-paper anchor: `paper/UNLIKELY_INTERSECTIONS_PREPRINT.md` (bridge and final strict margin).

## Goal
Identify the canonical endpoint representative with the ordinary problem-side target for `proving closure of atypical and anomalous intersection loci across admissible ambient families through a multi-lane unlikely-intersection super-architecture`.
...
4. the strict closure margin `M_UIN` remains positive.
...
If lock persistence, determining-class uniqueness, and strict coherence hold, then the endpoint representative is identified and the final margin `M_UIN` is meaningful.
