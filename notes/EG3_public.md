...
In-paper anchor: `paper/UNLIKELY_INTERSECTIONS_PREPRINT.md` (`UIN_G3`).

## Goal
Make the compactness/no-Zeno gate explicit for `proving closure of atypical and anomalous intersection loci across admissible ambient families through a multi-lane unlikely-intersection super-architecture`: near-failure behavior must be captured, not allowed to drift or accumulate invisibly.
...
`UIN_G3` closes when `kappa_compact` provides the compactness/no-collapse control: normalized near-failure families are precompact and admissible windows do not collapse.
This is the first-failure capture contribution to `M_UIN`.
...
If near-failure extraction and no-Zeno spacing hold, then `UIN_G3` prevents hidden drift before rigidity is applied.
